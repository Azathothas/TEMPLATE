...
fetch_list() {   # path outfile label
  ...
  while [ "$_page" -le 10 ]; do
    _code=$(fetch_proxy "$_p${_sep}per_page=100&page=$_page" "$_o.page")
    [ "$_code" = "200" ] || {
      gap "$_l: proxy returned $_code on page $_page"
      say "  $_l: http $_code"
      rm -f "$_o.parts" "$_o.page"
      return 1
    }
    _n=$(grep -o '"url"' "$_o.page" 2>/dev/null | wc -l | tr -d ' ')
    cat "$_o.page" >> "$_o.parts"
    [ "$_n" -lt 100 ] && break
    _page=$((_page + 1))
  done
  # Concatenated pages are several arrays; join them into one.
  if command -v node >/dev/null 2>&1; then
    ...
    ' "$_o.parts" "$_o" 2>/dev/null || cp "$_o.parts" "$_o"
  else
    # ⚠ WITHOUT node THE PAGES ARE LEFT AS THEY ARRIVED, several arrays in one
    # file, and PROVENANCE says so. Producing a file that LOOKS like one array
    # and is not is worse than an honest concatenation.
    cp "$_o.parts" "$_o"
    gap "$_l: node absent, pages left concatenated rather than joined"
  fi
  rm -f "$_o.parts" "$_o.page"
  say "  $_l: ok"
  return 0
