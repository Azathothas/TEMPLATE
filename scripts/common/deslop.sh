...
# -- WHAT IT IS FOR ----------------------------------------------------------
#
# A project adopts this template's engineering and wants none of the content
# written for a machine: a compliance rule forbids it, or the maintainer does
# not want it, which is a complete reason on its own.
#
# ⭐ THE INTENDED PATH IS TO NEVER INSTALL IT. docs/methodology/lean-adoption.md
# is a SELECTION made at adoption, and it is cheaper than any removal. This
# script is the retrofit for a project that already took everything.
#
...
# actually paid for. lean-adoption.md names all four. Read them, write them
# into the project's own contributing guide, THEN run this.
#
# Usage:
...
if [ "$APPLY" = "0" ]; then
  ...
  printf '\n'
  printf -- '⛔ Read docs/methodology/lean-adoption.md before removing any of this.\n'
  printf 'Four practices in docs/methodology/ are engineering rather than agent\n'
  printf 'instruction. Lift them into the project'"'"'s own contributing guide first.\n'
  printf '\n'
...
printf '%s\n' "$HITS" | while IFS= read -r f; do
  [ -n "$f" ] || continue
  git rm -q -- "$f" 2>/dev/null || rm -f -- "$f" 2>/dev/null || true
done
REMOVED=$NHITS

printf 'removed %s agent-facing file(s).\n\n' "$REMOVED"
