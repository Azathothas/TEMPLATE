...
```bash
for c in check-no-secrets check-docs check-placeholders; do curl -sSL -o "/tmp/$c.sh" "$TEMPLATE_RAW/scripts/common/$c.sh"; done
```
...
| path | what it gives you |
| --- | --- |
| `scripts/common/check-docs.sh` | links resolve, shell blocks parse, no control bytes |
